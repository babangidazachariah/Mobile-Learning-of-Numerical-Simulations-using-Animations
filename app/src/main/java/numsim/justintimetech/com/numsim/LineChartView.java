...
public class LineChartView extends ChartView {
    ...
    private ArrayList<PathOnChart> configurePaths(ArrayList<PathOnChart> paths) {
        ...
        no_of_x_negetive_unit = (float) minimum_x;
        no_of_x_positive_unit = (float) maximum_x;

        no_of_y_negetive_unit = (float) minimum_y;
        no_of_y_positive_unit = (float) maximum_y;
        maximumY =  no_of_y_positive_unit;
        maximumX = no_of_x_positive_unit;
    ...
    private void drawAxisNames(Canvas canvas) {
        ...
        Path path = new Path();
        //RectF rect = new RectF(offset_left*0.60f, y_translate_factor+(lineChartAttributes.Y_unit_name.length()*5), offset_left*0.10f, y_translate_factor-(lineChartAttributes.Y_unit_name.length()*5));
        RectF rect = new RectF((float) (xbl - offset_bottom*0.60), ybl, xtl - offset_top*0.60f, ytl);
        path.addRect(rect , Direction.CW);
        canvas.drawTextOnPath(lineChartAttributes.Y_unit_name, path, maximumY * 4, 0, paint);
        //canvas.drawTextOnPath(line, path, hOffset, vOffset, paint)
        path = new Path();
        //rect = new RectF(x_translate_factor-(lineChartAttributes.X_unit_name.length()*no_of_x_positive_unit), display_height-offset_bottom*no_of_x_positive_unit, x_translate_factor+(lineChartAttributes.X_unit_name.length()*no_of_x_positive_unit), display_height-offset_bottom*0.33f);
        rect = new RectF(xbl, (float) (ybl + offset_right*0.60), xbr, ybr + offset_right*0.60f);
        path.addRect(rect , Direction.CCW);
        canvas.drawTextOnPath(lineChartAttributes.X_unit_name, path, maximumX * 100,0 , paint);
        //canvas.drawText("x-Axis", display_width/2,display_height+45, paint);
        //canvas.drawText(minimum_y, locyAxisInPixels-40,canvasHeight/2, paint);
    ...
    private void drawPoints(Canvas canvas) {
        for (PathOnChart path : paths) {
            ...
            for (PointOnChart point : path.points) {
                ...
                canvas.drawText(""+point.x, (float)point.x+point.x, point.y, paint);
                canvas.drawText(""+point.x, (float)point.y+point.y, point.x, paint);
            }
        }
    }
